				...
				if (numUsers < MaxUsers)
				{
					try
					{
						...
						synchronized (this)
						{
						 ...
						 //TODO: Add handshaking. The server sends "Hello" and the client
						 //replies by "Hello". Only when the correct reply is received by
						 //the server, the client gets added to the list of clients.
							users[numUsers] = newSocket;
							BufferedReader tempReader =
													new BufferedReader(
																new InputStreamReader(newSocket.getInputStream()));
								PrintWriter tempWriter = new PrintWriter(new BufferedWriter(new OutputStreamWriter(newSocket.getOutputStream())));
								tempWriter.println("hello");
								tempWriter.flush();
								String handshak= tempReader.readLine();
								if(handshak.equals("hello")){
